	    ...
	    methods: {
	        ...
	        login: function login() {
	            ...
	            _leancloudStorage2.default.User.logIn(this.formData.username, this.formData.password).then(function (loginedUser) {
	                ...
	                _this2.fetchTodos();
	                console.log(_this2); //如果是function跟在then后面，this的值就是undefiened，这个this就是Vue$3
	            }, function (error) {
	                //箭头函数则会捕获该函数所在上下文的  this 值，作为自己的 this 值，这样就不用this = _this这样的写法了。
	                console.log('登陆失败');
	            });
	        },
	        getCurrentUser: function getCurrentUser() {
	            ...
	            if (current) {
	                var id = current.id,
	                    ...
	                    username = current.attributes.username; //解构赋值  我需要的几个参数放到前面的对象里，会自动去后面的对象中去找。
	        ...
	        saveTodos: function saveTodos() {
	            ...
	            //声明类型
	            var AVTodos = _leancloudStorage2.default.Object.extend('AllTodos'); //云端自动创建这个AllTodos类，在存储中可以看到
	            //新建对象
	            ...
	            acl.setReadAccess(_leancloudStorage2.default.User.current(), true); //文档中的setPublicReadAccsess出错
	        ...
	        updateTodos: function updateTodos() {
	            //更新对象有什么用？修改数据有用呀~未更新的字段不会产生变动
	            var dataString = JSON.stringify(this.todoList);
	            console.log(dataString); //一个对象数组，每个对象有title done createdAt三个属性
	            var avTodos = _leancloudStorage2.default.Object.createWithoutData('AllTodos', this.todoList.id); //这个ID就是上次saveTodos里面的id ，第一个参数是类名
	        ...
	        fetchTodos: function fetchTodos() {
	            ...
	            if (this.currentUser) {
	                ...
	                query.find().then(function (todos) {
	                    //这里为什么必须用箭头函数，不能用function (因为this执指向)
	                    var avAllTodos = todos[0]; //为什么是第一个 ，因为现在的逻辑是每个账户只有一个AllTodos类。存储方式为新建或更新。
	                    console.log(todos);
	                    var id = avAllTodos.id;
	                    _this4.todoList = JSON.parse(avAllTodos.attributes.content); //这个attributes从控制台看到的。
	                    _this4.todoList.id = id; //给这个数组设置id有什么用？
	                }, function (error) {
	                    console.log(error);
	                });
	            }
